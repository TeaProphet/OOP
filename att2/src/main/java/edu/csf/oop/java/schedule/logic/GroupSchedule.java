...
public class GroupSchedule {
    ...
    public void removeSubject(int dayNum, int subjectNum) throws WrongDayNumException, WrongSubjectNum {
        ...
        String subjectName = schedules.get(dayNum - 1).receiveSubject(subjectNum).getSubjectName();
    ...
    public boolean receiveIsNowLearning() throws WrongTimeCreated, WrongDayStringEntered {
        ...
        return dailySchedule.receiveIsNowLearning();
    }

    public String receiveCurrentSubject() throws SubjectNotFound, WrongTimeCreated, WrongDayStringEntered {
        String dayOfWeek = TimeUtils.getCurrentDayOfWeek();
        DailySchedule dailySchedule = schedules.get(TimeUtils.dayOfWeekToNum(dayOfWeek) - 1);
        logger.info("Trying to get current subject of group " + groupNumber + ", course " + courseNumber);
        return dailySchedule.receiveCurrentSubject().toString();
    }

    public String receiveWeeklySchedule() throws WrongDayNumException {
        ...
        for (int i = 0; i < schedules.size(); i++) {
            ...
            weeklySchedule.append(schedules.get(i).receiveStringSchedule()).append("\n");
        }
        return weeklySchedule.toString();
    }

    public int receiveNumberOfStudents() {
